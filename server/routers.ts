...
import { z } from "zod";
import { loginWithCredentials, registerWithCredentials } from "./_core/auth";
  ...
  auth: router({
    ...
    login: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const result = await loginWithCredentials(input, ctx.req, ctx.res);
        if (!result.success) {
          throw new TRPCError({
            code: "UNAUTHORIZED",
    ...
    register: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const result = await registerWithCredentials(input, ctx.req, ctx.res);
        if (!result.success) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: result.error,
